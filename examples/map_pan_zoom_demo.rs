...
use eframe::egui::{Vec2, Pos2, Rect, PointerButton};
...
fn main() {
    ...
    println!("   Initial camera position: {:?}", map_view.get_camera_position());
    ...
    println!("   Try to set zoom to 10.0 (clamped): {}\n", map_view.get_zoom_level());
    
    // Demonstrate Trackpad Gestures
    println!("3. TRACKPAD GESTURES");
    map_view.reset_view();
    println!("   Reset view - zoom: {}, camera: {:?}", 
        map_view.get_zoom_level(), map_view.get_camera_position());
    
    // Pinch zoom
    map_view.handle_pinch_gesture(Pos2::new(400.0, 300.0), 100.0, 150.0);
    println!("   After pinch zoom (100->150): {}", map_view.get_zoom_level());
    
    // Two-finger pan
    map_view.handle_two_finger_pan(Vec2::new(50.0, -30.0));
    println!("   After two-finger pan: {:?}\n", map_view.get_camera_position());
