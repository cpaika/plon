...
impl MonitoredPlonApp {
    ...
    fn generate_report(&self) {
        ...
        println!(
            "  • Mouse positions tracked: {}",
            data.mouse_positions.len()
        );
        ...
        for (time, view, delta) in &data.scroll_events {
            if view == "timeline" {
                println!(
                    "  • {:.2}s: Scroll delta ({:.2}, {:.2})",
                    time, delta.x, delta.y
                );
            }
        }
        ...
        std::process::exit(if data.unexpected_scrolls.is_empty() {
            0
        } else {
            1
        });
    ...
    fn update(&mut self, ctx: &egui::Context, frame: &mut eframe::Frame) {
        ...
        egui::Window::new("🧪 Scroll Test Monitor")
            ...
            .show(ctx, |ui| {
                ...
                ui.label(format!(
                    "Frame: {}",
                    self.detection_data.lock().unwrap().frame_count
                ));
