...
impl TimelineMouseTest {
    fn new() -> Self {
        ...
        let tasks: Vec<Task> = (0..50)
            .map(|i| {
                ...
                task.due_date =
                    Some(chrono::Utc::now() + chrono::Duration::days(((i % 30) + 5) as i64));
                task
            })
            .collect();
    ...
    fn track_changes(&mut self, ctx: &egui::Context, ui: &egui::Ui) {
        ...
        if let Some(last_rect) = self.last_rect {
            if (current_rect.size() - last_rect.size()).length() > 0.01 {
                results
                    .rect_changes
                    .push((last_rect, current_rect, timestamp));
            }
        }
    ...
    fn generate_report(&self) {
        ...
        println!(
            "  • Scroll changes detected: {}",
            results.scroll_changes.len()
        );
        println!(
            "  • Layout changes detected: {}",
            results.rect_changes.len()
        );

        println!("\n🖱️ Mouse Movement Analysis:");
        if !results.mouse_moves.is_empty() {
            let total_distance: f32 = results
                .mouse_moves
                .windows(2)
                .map(|w| (w[1].0 - w[0].0).length())
                .sum();
            println!(
                "  • Total mouse travel distance: {:.1} pixels",
                total_distance
            );
            println!(
                "  • Average movement per frame: {:.2} pixels",
...
impl eframe::App for TimelineMouseTest {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            if !results.unexpected_scrolls.is_empty() {
                ui.colored_label(
                    egui::Color32::RED,
                    format!(
                        "⚠️ {} unexpected scrolls detected!",
                        results.unexpected_scrolls.len()
                    ),
                );
            }
        });
        ...
        if self.test_phase == TestPhase::Complete {
            ...
            std::process::exit(if self.results.lock().unwrap().test_passed {
                0
            } else {
                1
            });
