...
fn main() {
        ...
        loop {
            ...
            if current_count == last_count && current_phase != TestPhase::Complete {
                ...
                if freeze_start.is_none() {
                    ...
                    println!("⚠️  Warning: No frames for 100ms during {:?}", current_phase);
                }
                
                let freeze_duration = freeze_start.unwrap().elapsed();
                
                if freeze_duration > Duration::from_millis(500) {
                    println!("❌ FREEZE DETECTED: No frames for {:?} during {:?}!", 
    ...
    let result = eframe::run_native(
        ...
        Box::new(move |cc| {
            ...
            let mut app = PlonApp::new(cc, repository);
    ...
    fn update(&mut self, ctx: &egui::Context, frame: &mut eframe::Frame) {
        ...
        *self.last_frame_time.lock() = Instant::now();
        ...
        if self.freeze_detected.load(Ordering::Relaxed) {
            println!("Freeze detected at frame {} during {:?}", frame_num, current_phase);
            ctx.send_viewport_cmd(egui::ViewportCommand::Close);
        }
...
impl TestHarness {
    ...
    fn simulate_pan(&mut self, ctx: &egui::Context, dx: f32, dy: f32) {
        ...
        ctx.input_mut(|i| {
            ...
            i.events.push(egui::Event::PointerMoved(self.mouse_position));
