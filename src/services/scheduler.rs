...
impl RecurringTaskScheduler {
    ...
    pub async fn start(self) {
        ...
        info!("Starting recurring task scheduler with {}min interval", self.interval_minutes);
    ...
    pub async fn get_upcoming(&self, days_ahead: i64) -> Result<Vec<(String, chrono::DateTime<chrono::Utc>)>> {
        let templates = self.recurring_service.get_upcoming_occurrences(days_ahead).await?;
        
        let upcoming = templates
            .into_iter()
            .filter_map(|t| {
                t.next_occurrence.map(|next| (t.title, next))
            })
            .collect();
...
mod tests {
    ...
    async fn test_scheduler_run_once() {
        ...
        recurring_service.create_recurring_template(
            "Test Task".to_string(),
