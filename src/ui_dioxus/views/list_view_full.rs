    ...
    rsx! {
        div {
            ...
            div {
                ...
                // Status filter
                select {
                    value: "{selected_status.read().as_ref().map(|s| format!(\"{:?}\", s)).unwrap_or(\"All\".to_string())}",
                    onchange: move |evt| {
...
fn TaskListItem(
    ...
    rsx! {
        div {
            class: if selected { "task-item selected" } else { "task-item" },
            style: format!("padding: 15px; border-bottom: 1px solid #eee; cursor: pointer; 
                           background: {}; transition: background 0.2s;",
                           if selected { "#f0f8ff" } else { "white" }),
            ...
            div {
                ...
                // Status icon
                span {
                    style: format!("font-size: 24px; color: {};", status_color),
                    "{status_icon}"
                }
                ...
                div {
                    ...
                    div {
                        ...
                        span {
                            style: format!("padding: 2px 8px; border-radius: 4px; font-size: 11px; 
                                          font-weight: bold; color: white; background: {};",
                                          priority_color),
                            "{task.priority:?}"
                        }
                    ...
                    div {
                        ...
                        // Created date
                        span {
                            "Created: {task.created_at.format(\"%Y-%m-%d\")}"
                        }
                ...
                div {
                    ...
                    button {
                        onclick: move |evt| {
                            evt.stop_propagation();
                            if js_sys::eval("confirm('Delete this task?')").unwrap().as_bool().unwrap_or(false) {
                                ondelete.call(evt);
                            }
                        },
