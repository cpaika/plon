        ...
        div {
            ...
            div {
                ...
                onmousemove: move |evt| {
                    // Get the actual position relative to the map container
                    let rect = evt.client_coordinates();
                    mouse_position.set((rect.x as f64, rect.y as f64));
                },
                ...
                svg {
                    ...
                    // Define gradients and arrowhead markers
                    defs {
                        // Gradient for dependency lines
                        linearGradient {
                            id: "dependency-gradient",
                            x1: "0%", y1: "0%", x2: "100%", y2: "0%",
                            stop { offset: "0%", "stop-color": "#666", "stop-opacity": "0.8" }
                            stop { offset: "100%", "stop-color": "#999", "stop-opacity": "0.8" }
                        }
                        
                        linearGradient {
                            id: "dependency-gradient-hover",
                            x1: "0%", y1: "0%", x2: "100%", y2: "0%",
                            stop { offset: "0%", "stop-color": "#4CAF50", "stop-opacity": "1" }
                            stop { offset: "100%", "stop-color": "#66BB6A", "stop-opacity": "1" }
                        }
                        
                        // Better arrowhead design
                        marker {
                            id: "arrowhead",
                            "markerWidth": "12",
                            "markerHeight": "12",
                    ...
                    // Render dependencies with animation
                    for dep in dependencies.read().clone() {
                        if let (Some(from_task), Some(to_task)) = (
                        ...
                        ) {
                            {
                                ...
                                // Better curved path calculation
                                let distance = ((x2 - x1).powi(2) + (y2 - y1).powi(2)).sqrt();
                                let ctrl_offset = (distance / 4.0).min(100.0).max(30.0);
                                ...
                                let stroke = if is_hovered { "url(#dependency-gradient-hover)" } else { "url(#dependency-gradient)" };
                                let stroke_width = if is_hovered { "3" } else { "2" };
                                let marker = if is_hovered { "url(#arrowhead-hover)" } else { "url(#arrowhead)" };
                                let animation = if is_hovered { 
                                    "animation: stroke-dashoffset 1s linear infinite"
                                } else { 
                                    "animation: stroke-dashoffset 2s linear infinite" 
                                };
                                ...
                                rsx! {
                                    ...
                                    path {
                                        ...
                                        "stroke-linecap": "round",
                                        style: "pointer-events: stroke; cursor: pointer; {animation}",
                    ...
                    if let Some(drag_state) = dragging_connection.read().as_ref() {
                        {
                            ...
                            rsx! {
                                // Preview line with animation
                                path {
                                    d: "{path}",
                                    stroke: "url(#dependency-gradient-hover)",
                                    "stroke-width": "2",
                                    "stroke-dasharray": "8,4",
                                    fill: "none",
                                    "marker-end": "url(#arrowhead-hover)",
                                    "stroke-linecap": "round",
                                    style: "animation: stroke-dashoffset 1s linear infinite",
                                    opacity: "0.8"
                                }
                            }
                        }
    ...
    rsx! {
        div {
            ...
            // Left connection node (input)
            {
                ...
                let transform = if is_left_node_hover { "translateY(-50%) scale(1.1)" } else { "translateY(-50%)" };
                let box_shadow = if is_left_node_hover { "0 4px 8px rgba(0,0,0,0.3)" } else { "0 2px 4px rgba(0,0,0,0.2)" };
                
                rsx! {
                    div {
                        style: "position: absolute; left: -8px; top: 50%; transform: {transform};
