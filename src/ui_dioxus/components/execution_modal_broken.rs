...
fn ExecutionContent(execution: TaskExecution) -> Element {
    ...
    let has_pr = execution.pr_url.is_some();
    let pr_url = execution.pr_url.clone().unwrap_or_default();
    
    let has_error = execution.error_message.is_some();
    let error_msg = execution.error_message.clone().unwrap_or_default();
    
    let has_completed = execution.completed_at.is_some();
