...
impl ResourceView {
    ...
    pub fn show(&mut self, ui: &mut Ui, resources: &mut Vec<Resource>) {
        ...
        if self.show_edit_dialog
            && let Some(resource_id) = self.selected_resource
        {
            egui::Window::new("Edit Resource")
                ...
                .show(ui.ctx(), |ui| {
                    ...
                    ui.horizontal(|ui| {
                        if ui.button("Save").clicked() && !self.edit_resource_name.trim().is_empty()
                        {
                            // Find and update the resource
                            if let Some(resource) =
                                resources.iter_mut().find(|r| r.id == resource_id)
                            {
