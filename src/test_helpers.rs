...
use crate::services::{GoalService, ResourceService, TaskConfigService, TaskService};
...
impl PlonApp {
    pub fn new_for_test() -> Self {
        ...
        Self {
            ...
            claude_code_view: crate::ui::views::claude_code_view::ClaudeCodeView::new(),
    ...
    pub fn kanban_quick_add_task(&mut self, column_index: usize, title: String) {
        ...
        if let Some(new_task) = self.kanban_view.tasks.last()
            && !self.tasks.iter().any(|t| t.id == new_task.id)
        {
    ...
    pub fn get_kanban_view_mut(
        &mut self,
    ) -> &mut crate::ui::views::kanban_view_enhanced::KanbanView {
