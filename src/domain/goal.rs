...
mod tests {
    ...
    fn test_calculate_progress() {
        ...
        let statuses = vec![(task_id1, true), (task_id2, false), (task_id3, true)];

        let progress = goal.calculate_progress(&statuses);
        assert!((progress - 66.66667).abs() < 0.001);

        let all_complete = vec![(task_id1, true), (task_id2, true), (task_id3, true)];
        assert_eq!(goal.calculate_progress(&all_complete), 100.0);

        let none_complete = vec![(task_id1, false), (task_id2, false), (task_id3, false)];
