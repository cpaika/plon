...
fn test_timeline_infinite_scroll_via_screenshots() {
    ...
    assert!(!is_scrolling, "Infinite scrolling detected in timeline view!");
}
...
fn analyze_screenshots_for_scrolling(screenshots: &[String]) -> bool {
    ...
    for i in 1..screenshots.len() {
        ...
        println!("Difference between screenshot {} and {}: {:.2}%", i-1, i, diff * 100.0);
    }
    ...
    if high_diff_count >= differences.len() - 1 {
        println!("DETECTED: Continuous changes across all screenshots indicate infinite scrolling!");
        return true;
    }
...
fn compare_images(path1: &str, path2: &str) -> f32 {
    ...
    match output {
        ...
        Err(_) => {
            println!("Warning: ImageMagick 'compare' command not found. Using file size comparison.");
...
fn test_timeline_redraw_frequency() {
    ...
    if !samples.is_empty() {
        ...
        assert!(avg_cpu < 50.0, "High CPU usage detected ({:.2}%), likely due to infinite scrolling!", avg_cpu);
        assert!(max_cpu < 80.0, "Peak CPU usage too high ({:.2}%), likely due to infinite scrolling!", max_cpu);
    }
}