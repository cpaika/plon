...
mod task_tests {
    ...
    fn test_task_tags_and_metadata() {
        ...
        // Add metadata
        task.metadata
            .insert("category".to_string(), "feature".to_string());
        task.metadata
...
mod dependency_tests {
    ...
    fn test_topological_sort() {
        let tasks: Vec<_> = (0..5)
            .map(|i| Task::new(format!("Task {}", i), "".to_string()))
            .collect();
    ...
    fn test_critical_path() {
        ...
        let estimates: HashMap<Uuid, f32> = tasks.iter().map(|t| (t.id, 8.0)).collect();
...
mod edge_case_tests {
    ...
    fn test_unicode_handling() {
        ...
        let goal = Goal::new("目标 🎯".to_string(), "Цель με emoji 🏆".to_string());
...
async fn test_database_integration() {
    ...
    task.metadata
        .insert("category".to_string(), "integration".to_string());
