...
use plon::services::{GoalService, ResourceService, TaskService};
use std::sync::Arc;
...
async fn test_dashboard_overdue_tasks() {
    ...
    // Check overdue tasks
    let all_tasks = service.list_all().await.unwrap();
    let overdue_tasks: Vec<_> = all_tasks.iter().filter(|t| t.is_overdue()).collect();
...
async fn test_dashboard_resource_utilization() {
    ...
    let overloaded: Vec<_> = all_resources.iter().filter(|r| r.is_overloaded()).collect();
...
async fn test_dashboard_task_age_analysis() {
    ...
    let stale_tasks: Vec<_> = all_tasks
        .iter()
        .filter(|t| {
            t.status != TaskStatus::Done && now.signed_duration_since(t.created_at).num_days() > 30
        })
        .collect();
