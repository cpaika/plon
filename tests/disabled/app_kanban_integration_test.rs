...
fn test_kanban_view_is_using_improved_version() {
    ...
    assert!(
        app.has_improved_kanban_view(),
        "App should use the improved Kanban view"
    );
}
...
fn test_kanban_drag_drop_in_main_app() {
    ...
    assert!(
        app.can_start_drag_in_kanban(task1.id),
    ...
    assert!(
        app.is_kanban_dragging(),
        "Should be dragging in Kanban view"
    );
...
fn test_kanban_view_renders_properly() {
    ...
    assert_eq!(
        app.get_kanban_tasks_in_column(0).len(),
        1,
        "Todo column should have 1 task"
    );
    assert_eq!(
        app.get_kanban_tasks_in_column(1).len(),
        1,
        "In Progress column should have 1 task"
    );
    assert_eq!(
        app.get_kanban_tasks_in_column(3).len(),
        1,
...
fn test_kanban_quick_add_in_app() {
    ...
    assert!(
        app.is_kanban_quick_add_active(0),
        "Quick add should be active"
    );
