...
async fn test_create_recurring_template() {
    ...
    assert_eq!(
        fetched_template.recurrence_rule.pattern,
...
async fn test_complex_recurrence_rules() {
    ...
    assert_eq!(
        fetched_yearly.recurrence_rule.pattern,
    ...
    assert_eq!(
        fetched_weekly.recurrence_rule.pattern,
...
async fn test_metadata_and_resources() {
    ...
    template
        .metadata
        .insert("type".to_string(), "review".to_string());
    ...
    assert_eq!(
        fetched.metadata.get("project"),
        Some(&"backend".to_string())
    );
    assert_eq!(
        fetched.metadata.get("team"),
