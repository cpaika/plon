...
fn test_no_duplicate_scroll_area_ids() {
    ...
    for (idx, column) in kanban.columns.iter().enumerate() {
        ...
        assert!(
            !scroll_ids.contains(&expected_id),
            "Duplicate ScrollArea ID found for column {}",
            column.title
        );
...
fn test_text_rendering_without_overlap() {
    ...
    assert!(
        card_height >= 80.0,
        "Card height should be sufficient for content"
    );

    // With tags and description, height should be increased
    assert!(
        card_height > 80.0,
        "Card with tags should have increased height"
    );
}
...
fn test_special_characters_in_text() {
    ...
    assert_eq!(
        kanban.tasks.len(),
        5,
        "All tasks with special characters should be added"
    );
}
...
fn test_proper_text_truncation() {
    ...
    assert!(
        card_height <= 200.0,
        "Card height should be capped at maximum"
    );
}
