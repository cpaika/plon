...
fn test_timeline_scroll_stability() {
    ...
    assert!(
        timeline_view.zoom_level > 0.0 && timeline_view.zoom_level <= 5.0,
        "Zoom level should be bounded: {}",
        timeline_view.zoom_level
    );

    assert!(
        timeline_view.days_to_show > 0 && timeline_view.days_to_show <= 365,
        "Days to show should be reasonable: {}",
        timeline_view.days_to_show
    );
...
fn test_timeline_zoom_bounds() {
    ...
    assert!(
        timeline_view.zoom_level >= 0.5,
        "Zoom should have minimum bound"
    );
    assert!(
        timeline_view.zoom_level <= 3.0,
        "Zoom should have maximum bound"
    );
