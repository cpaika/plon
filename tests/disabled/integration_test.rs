...
fn test_complete_workflow() {
    ...
    let mut task1 = Task::new(
        "Design UI".to_string(),
        "Create mockups for the new dashboard".to_string(),
    );
    let mut task2 = Task::new(
        "Implement Backend".to_string(),
        "Set up API endpoints".to_string(),
    );
    ...
    let task2_deps = dep_graph.get_dependencies(task2.id);
    assert_eq!(task2_deps.len(), 1);
    assert_eq!(task2_deps[0].0, task1.id);

    // Get dependents for task2
    let task2_dependents = dep_graph.get_dependents(task2.id);
    assert_eq!(task2_dependents.len(), 1);
    assert_eq!(task2_dependents[0].0, task3.id);
...
fn test_dependency_cycle_detection() {
    ...
    // Trying to create a cycle: 3 -> 1 should fail
    let result = dep_graph.add_dependency(&Dependency::new(
        task3_id,
        task1_id,
        DependencyType::FinishToStart,
    ));
