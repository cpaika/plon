...
fn test_intensive_panning_performance() {
    ...
    assert!(
        avg_time_per_op < 1.0,
        "Pan operations too slow: {}ms average",
        avg_time_per_op
    );
}
...
fn test_trackpad_scroll_performance() {
    ...
    for i in 0..5000 {
        let delta = egui::Vec2::new((i as f32 % 20.0) - 10.0, (i as f32 % 30.0) - 15.0);
    ...
    let elapsed = start_time.elapsed();
    println!(
        "Performed {} scroll operations in {:?}",
        operations, elapsed
    );
    ...
    assert!(
        avg_time_per_op < 0.5,
        "Scroll operations too slow: {}ms average",
        avg_time_per_op
    );
}
...
fn test_zoom_performance() {
    ...
    assert!(
        avg_time_per_op < 0.5,
        "Zoom operations too slow: {}ms average",
        avg_time_per_op
    );
}
...
fn test_combined_stress() {
    ...
    for cycle in 0..100 {
        ...
        if current_elapsed > Duration::from_secs(5) {
            panic!(
                "Operations taking too long - potential beachball condition detected after {} operations",
                operations
            );
        }
    }
    ...
    let elapsed = start_time.elapsed();
    println!(
        "Performed {} combined operations in {:?}",
        operations, elapsed
    );
