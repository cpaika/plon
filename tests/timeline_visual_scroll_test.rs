use eframe::{egui, NativeOptions, Frame};
...
impl TimelineVisualTestApp {
    ...
    fn capture_frame_state(&self, ctx: &egui::Context) -> FrameCapture {
        ...
        let scroll_position = ctx.memory(|mem| {
            ...
            let scroll_pos = mem.data.get_temp::<egui::Vec2>(egui::Id::new("scroll_pos"))
    ...
    fn detect_auto_scrolling(&self) -> Option<String> {
        ...
        for window in recent_captures.windows(2) {
            ...
            let rect_changed = 
                (curr.visible_rect.min - prev.visible_rect.min).length() > 0.1 ||
...
impl eframe::App for TimelineVisualTestApp {
    fn update(&mut self, ctx: &egui::Context, frame: &mut Frame) {
        ...
        self.frame_captures.lock().unwrap().push(frame_capture.clone());
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.label(format!("Frames captured: {}", self.frame_captures.lock().unwrap().len()));
            ui.label(format!("Frames since interaction: {}", self.frames_since_last_interaction));
            ...
            if self.test_phase == TestPhase::Monitoring {
                if let Some(error) = self.detect_auto_scrolling() {
                    ...
                    println!("AUTO-SCROLL DETECTED AT FRAME {}", frame_capture.frame_number);
        ...
        if self.test_phase != TestPhase::Complete {
            ...
            if auto_scroll_events > 0 {
                println!("❌ FAIL: Detected {} auto-scroll events", auto_scroll_events);
            } else {
