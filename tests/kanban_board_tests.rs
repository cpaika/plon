use plon::ui::views::kanban_view_improved::{KanbanView, KanbanColumn};
...
mod kanban_drag_drop_tests {
    ...
    fn test_kanban_initialization() {
        ...
        assert!(!kanban.columns.is_empty(), "Kanban should have default columns");
        
        // Should have standard columns
        let column_titles: Vec<String> = kanban.columns.iter()
            .map(|c| c.title.clone())
            .collect();
        
        assert!(column_titles.contains(&"To Do".to_string()), "Should have To Do column");
        assert!(column_titles.contains(&"In Progress".to_string()), "Should have In Progress column");
        assert!(column_titles.contains(&"Done".to_string()), "Should have Done column");
    }
    ...
    fn test_drag_start() {
        ...
        assert_eq!(kanban.get_dragging_task_id(), Some(task.id), "Should track dragging task ID");
    }
    ...
    fn test_drag_to_different_column() {
        ...
        let updated_task = kanban.tasks.iter()
            .find(|t| t.id == task.id)
            .expect("Task should exist");
        
        assert_eq!(updated_task.status, TaskStatus::InProgress, "Task should be in In Progress status");
    ...
    fn test_cancel_drag() {
        ...
        kanban.cancel_drag();
        assert!(!kanban.is_dragging(), "Should no longer be dragging after cancel");
    }
    ...
    fn test_drop_zones() {
        ...
        assert!(kanban.is_over_column(egui::Pos2::new(150.0, 200.0), 0), "Should be over Todo column");
        
        // Test if position is in In Progress column  
        assert!(kanban.is_over_column(egui::Pos2::new(450.0, 200.0), 1), "Should be over In Progress column");
        
        // Test if position is outside any column
        assert!(!kanban.is_over_column(egui::Pos2::new(-50.0, 200.0), 0), "Should not be over any column");
    }
...
mod kanban_rendering_tests {
    ...
    fn test_card_height_calculation() {
        let kanban = KanbanView::new();
        let task = create_test_task("Test task with a longer description that might wrap", TaskStatus::Todo);
    ...
    fn test_column_header_rendering() {
        ...
        for column in &kanban.columns {
            ...
            assert!(column.color != egui::Color32::BLACK, "Column should have color");
        }
    }
    ...
    fn test_wip_limits() {
        ...
        for i in 0..4 {
            kanban.add_task(create_test_task(&format!("Task {}", i), TaskStatus::InProgress));
        }
        
        assert!(kanban.is_column_over_wip_limit(1), "Column should be over WIP limit");
    }
    ...
    fn test_empty_column_message() {
        ...
        assert!(!empty_message.is_empty(), "Should have empty column message");
    }
    ...
    fn test_responsive_columns() {
        ...
        assert!(kanban.should_stack_columns(), "Should stack columns on narrow screen");
        
        // Test with wide screen
        kanban.update_layout(1400.0);
        assert!(!kanban.should_stack_columns(), "Should not stack columns on wide screen");
    }
    ...
    fn test_card_colors_by_priority() {
        ...
        assert_ne!(high_color, low_color, "Different priorities should have different colors");
    }
    ...
    fn test_overdue_task_highlighting() {
        ...
        assert!(kanban.should_highlight_as_overdue(&overdue_task), "Should highlight overdue task");
        
        let mut future_task = create_test_task("Future", TaskStatus::Todo);
        future_task.due_date = Some(Utc::now() + chrono::Duration::days(7));
        
        assert!(!kanban.should_highlight_as_overdue(&future_task), "Should not highlight future task");
    }
}
...
mod kanban_interaction_tests {
    ...
    fn test_column_collapse() {
        ...
        assert!(!kanban.is_column_collapsed(0), "Column should not be collapsed initially");
        
        kanban.toggle_column_collapse(0);
        assert!(kanban.is_column_collapsed(0), "Column should be collapsed");
        
        kanban.toggle_column_collapse(0);
        assert!(!kanban.is_column_collapsed(0), "Column should be expanded again");
    }
    ...
    fn test_search_filter() {
        ...
        assert_eq!(visible_tasks.len(), 1, "Should only show tasks matching search");
    ...
    fn test_keyboard_shortcuts() {
        ...
        assert_eq!(updated_task.status, TaskStatus::InProgress, "Task should move right");
    }
}