...
"#;
    ...
    assert_eq!(
        created.subtasks[0].description,
        "Setup development environment"
    );
...
async fn test_task_editor_dependencies() {
    ...
    // Create tasks
    let task1 = service
        .create(Task::new("Task 1".to_string(), "".to_string()))
        .await
        .unwrap();
    let task2 = service
        .create(Task::new("Task 2".to_string(), "".to_string()))
        .await
        .unwrap();
    let task3 = service
        .create(Task::new("Task 3".to_string(), "".to_string()))
        .await
        .unwrap();
    ...
    let task2_deps = repository
        .dependencies
        .get_dependencies_for_task(task2.id)
        .await
        .unwrap();
    assert_eq!(task2_deps.len(), 0); // Stub returns empty

    let task2_dependents = repository
        .dependencies
        .get_dependents_for_task(task2.id)
        .await
        .unwrap();
