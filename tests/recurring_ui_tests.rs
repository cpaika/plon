...
fn test_recurring_editor_build_template() {
    ...
    assert_eq!(template.recurrence_rule.time_of_day, NaiveTime::from_hms_opt(16, 0, 0).unwrap());
}
...
fn test_pattern_display_string() {
    ...
    assert_eq!(editor.pattern_to_string(RecurrencePattern::Weekly), "Weekly");
    assert_eq!(editor.pattern_to_string(RecurrencePattern::Monthly), "Monthly");
    assert_eq!(editor.pattern_to_string(RecurrencePattern::Yearly), "Yearly");
    assert_eq!(editor.pattern_to_string(RecurrencePattern::Custom), "Custom");
}
...
fn test_frequency_description() {
    ...
    assert_eq!(editor.get_frequency_description(), "Every week on Mon, Wed, Fri");
    
    editor.interval = 2;
    assert_eq!(editor.get_frequency_description(), "Every 2 weeks on Mon, Wed, Fri");
    
    // Monthly
    editor.pattern = RecurrencePattern::Monthly;
    editor.interval = 1;
    editor.day_of_month = Some(15);
    assert_eq!(editor.get_frequency_description(), "Every month on the 15th");
    ...
    assert_eq!(editor.get_frequency_description(), "Every year on December 25");
}