...
use eframe::egui::{self, Vec2, Pos2, Rect, PointerButton};
use std::sync::Arc;
...
fn test_pan_with_shift_left_click() {
    ...
    ctx.map_view.start_pan_with_modifiers(start_pos, PointerButton::Primary, true, false);
    ctx.map_view.update_pan(end_pos);
    ctx.map_view.end_pan();
...
fn test_pan_accumulates_multiple_drags() {
    ...
    // First pan
    ctx.map_view.start_pan(Pos2::new(100.0, 100.0), PointerButton::Middle);
    ...
    // Second pan
    ctx.map_view.start_pan(Pos2::new(200.0, 200.0), PointerButton::Middle);
...
fn test_zoom_in_with_scroll() {
    ...
    ctx.map_view.handle_scroll(scroll_delta, Pos2::new(400.0, 300.0));
...
fn test_trackpad_pinch_zoom() {
    ...
    ctx.map_view.handle_pinch_gesture(pinch_center, initial_distance, final_distance);
...
fn test_viewport_culling_with_pan_zoom() {
    ...
    // Pan to task 2
    ctx.map_view.set_camera_position(Vec2::new(-4600.0, -4700.0));
...
fn test_pan_performance_with_many_tasks() {
    ...
    assert!(elapsed.as_millis() < 100, "Pan operations took too long: {:?}", elapsed);
}
...
fn test_zoom_performance_with_many_tasks() {
    ...
    // Should complete in reasonable time
    assert!(elapsed.as_millis() < 100, "Zoom operations took too long: {:?}", elapsed);
}