...
async fn test_kanban_wip_limits() {
    ...
    // Define WIP limits
    let wip_limits = HashMap::from([(TaskStatus::InProgress, 3), (TaskStatus::Review, 2)]);
...
async fn test_kanban_card_details() {
    ...
    let mut task = Task::new(
        ...
        "Task with all details".to_string(),
    );
...
async fn test_kanban_quick_add() {
    ...
    for status in columns {
        let column_tasks: Vec<_> = all_tasks.iter().filter(|t| t.status == status).collect();
