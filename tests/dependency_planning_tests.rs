...
fn test_project_duration_calculation() {
    ...
    let mut tasks = vec![
        ...
        Task::new("Implementation".to_string(), "Implementation phase".to_string()),
...
fn test_parallel_tasks_duration() {
    ...
    graph.add_dependency(&Dependency::new(tasks[0].id, tasks[1].id, DependencyType::FinishToStart)).unwrap();
    graph.add_dependency(&Dependency::new(tasks[0].id, tasks[2].id, DependencyType::FinishToStart)).unwrap();
    graph.add_dependency(&Dependency::new(tasks[1].id, tasks[3].id, DependencyType::FinishToStart)).unwrap();
    graph.add_dependency(&Dependency::new(tasks[2].id, tasks[3].id, DependencyType::FinishToStart)).unwrap();
...
fn test_blocked_tasks_detection() {
    ...
    let blocked_tasks: Vec<_> = all_tasks.iter()
...
fn test_dependency_types_affect_scheduling() {
    ...
    graph.add_dependency(&Dependency::new(task1, task2, DependencyType::FinishToStart)).unwrap();
    graph.add_dependency(&Dependency::new(task1, task3, DependencyType::StartToStart)).unwrap();
    graph.add_dependency(&Dependency::new(task2, task4, DependencyType::FinishToFinish)).unwrap();
...
fn test_resource_allocation_with_dependencies() {
    ...
    let mut graph = DependencyGraph::new();
    graph.add_dependency(&Dependency::new(task1.id, task3.id, DependencyType::FinishToStart)).unwrap();
    graph.add_dependency(&Dependency::new(task2.id, task3.id, DependencyType::FinishToStart)).unwrap();
...
fn test_estimate_adjustment_with_dependencies() {
    ...
    let mut graph = DependencyGraph::new();
    graph.add_dependency(&Dependency::new(tasks[0].id, tasks[1].id, DependencyType::FinishToStart)).unwrap();
    graph.add_dependency(&Dependency::new(tasks[1].id, tasks[2].id, DependencyType::FinishToStart)).unwrap();
    ...
    let critical_path = graph.get_critical_path(&estimates);
    let raw_duration: f32 = critical_path.iter()
        .map(|id| estimates.get(id).unwrap_or(&0.0))
        .sum();
