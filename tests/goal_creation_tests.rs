...
mod goal_creation_tests {
    ...
    fn test_goal_creation_with_parent() {
        let parent = Goal::new(
            ...
            "Goals for the year".to_string()
        );
        
        let mut child = Goal::new(
            "Q1 Goals".to_string(),
    ...
    fn test_goal_hierarchy() {
        ...
        // Verify hierarchy
        let children: Vec<&Goal> = goals.iter()
    ...
    fn test_goal_validation() {
        ...
        assert!(goal.title.is_empty(), "Should allow empty title but mark as invalid");
    ...
    async fn test_goal_service_create_with_validation() {
        ...
        assert!(result.is_ok(), "Failed to create goal via service: {:?}", result.err());
