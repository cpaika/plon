...
fn test_actual_drag_drop_workflow() {
    ...
    let mut in_progress_task = Task::new("Feature X".to_string(), "Implement feature X".to_string());
    ...
    assert_eq!(in_progress_tasks.len(), 1, "Should have 1 task in In Progress");
    ...
    assert_eq!(in_progress_tasks_after.len(), 2, "In Progress should have 2 tasks");
    
    // Verify task status changed
    let moved_task = app.get_task(todo_id).expect("Task should exist");
    assert_eq!(moved_task.status, TaskStatus::InProgress, "Task status should be updated");
}
...
fn test_kanban_column_interactions() {
    ...
    // Test column count
    assert_eq!(app.get_kanban_column_count(), 4, "Should have 4 default columns");
    
    // Test WIP limits
    let kanban = app.get_kanban_view();
    assert!(kanban.columns[1].wip_limit.is_some(), "In Progress should have WIP limit");
    assert_eq!(kanban.columns[1].wip_limit, Some(3), "In Progress WIP limit should be 3");
    ...
    let kanban = app.get_kanban_view();
    assert!(kanban.is_column_over_wip_limit(1), "Should be over WIP limit");
}
...
fn test_kanban_selection_and_multi_select() {
    ...
    assert_eq!(kanban.selected_tasks.len(), 3, "Should have 3 selected tasks");
    ...
    assert_eq!(in_progress_tasks.len(), 3, "All tasks should be in In Progress");
}
...
fn test_kanban_search_and_filter() {
    ...
    app.add_test_task(Task::new("Fix login bug".to_string(), "Critical issue".to_string()));
    app.add_test_task(Task::new("Add feature".to_string(), "New feature request".to_string()));
    app.add_test_task(Task::new("Update docs".to_string(), "Documentation update".to_string()));
...
fn test_kanban_quick_add_comprehensive() {
    ...
    for column_idx in 0..4 {
        ...
        assert!(tasks.iter().any(|t| t.title == title), 
                "Should have added task to column {}", column_idx);
    }
}
...
fn test_kanban_keyboard_navigation() {
    ...
    // Check task moved
    {
        ...
        assert_eq!(task_after_right.status, TaskStatus::InProgress, "Should move to In Progress");
    }
    ...
    // Check task moved back
    {
        ...
        assert_eq!(task_after_left.status, TaskStatus::Todo, "Should move back to Todo");
    }
}
...
fn test_kanban_column_collapse() {
    ...
    assert!(!kanban.is_column_collapsed(0), "Should not be collapsed initially");
...
fn test_kanban_task_details() {
    ...
    let mut task = Task::new("Detailed task".to_string(), "With lots of metadata".to_string());
    ...
    assert!(kanban.should_highlight_as_overdue(&overdue_task), "Should highlight overdue");
}
...
fn test_kanban_responsive_layout() {
    ...
    assert!(!kanban.should_stack_columns(), "Should not stack on wide screen");
    
    let width = kanban.calculate_column_width(1400.0);
    assert!(width >= 320.0 && width <= 400.0, "Column width should be in reasonable range");
    
    // Test mobile layout
    kanban.update_layout(600.0);
    assert!(kanban.should_stack_columns(), "Should stack on narrow screen");
}
...
fn test_kanban_full_user_workflow() {
    ...
    let bug_task = app.get_tasks().iter().find(|t| t.title.contains("bug")).unwrap();
