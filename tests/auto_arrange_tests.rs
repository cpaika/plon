...
mod tests {
    ...
    fn test_parallel_dependencies_layout() {
        ...
        assert_parallel_dependency_layout(&tasks, start_task_id, vec![parallel1_id, parallel2_id, parallel3_id], end_task_id);
    }
    ...
    fn assert_tasks_grouped_by_status(tasks: &[Task]) {
        ...
        assert!(same_status_distance < diff_status_distance * 0.7,
                "Tasks with same status should be closer together");
    }
    ...
    fn assert_tasks_grouped_by_tags(tasks: &[Task]) {
        ...
        assert!(same_tag_distance < diff_tag_distance * 0.7,
                "Tasks with same tags should be closer together");
    }
    ...
    fn assert_tasks_grouped_by_priority(tasks: &[Task]) {
        ...
        assert!(same_priority_distance < diff_priority_distance * 0.7,
    ...
    fn assert_dependency_chain_layout(tasks: &[Task], expected_order: Vec<Uuid>) {
        ...
        for i in 0..expected_order.len() - 1 {
            ...
            let task2 = tasks.iter().find(|t| t.id == expected_order[i + 1]).unwrap();
            
            assert!(task1.position.x < task2.position.x,
                    "Task {} should be to the left of Task {}", i, i + 1);
        }
    }
    
    fn assert_parallel_dependency_layout(tasks: &[Task], start_id: Uuid, parallel_ids: Vec<Uuid>, end_id: Uuid) {
        ...
        for task in tasks {
            if parallel_ids.contains(&task.id) {
                assert!(task.position.x > start_task.position.x,
                        "Parallel tasks should be to the right of start task");
                assert!(task.position.x < end_task.position.x,
    ...
    fn assert_topological_ordering(tasks: &[Task], task_ids: &[Uuid]) {
        ...
        assert!(sorted_tasks.windows(2).all(|w| w[0].position.x <= w[1].position.x));
    }
        ...
        for i in 0..tasks.len() {
            for j in i + 1..tasks.len() {
                ...
                let distance = ((task1.position.x - task2.position.x).powi(2) +
                               (task1.position.y - task2.position.y).powi(2)).sqrt();
                
                assert!(distance >= min_spacing,
                        "Tasks {} and {} are too close (distance: {})", i, j, distance);
            }
        }
    }
    ...
    fn calculate_average_distance_diff_status(tasks: &[Task]) -> f64 {
        ...
        if count > 0 { total_distance / count as f64 } else { f64::MAX }
    }
