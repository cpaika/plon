...
fn test_nested_allocations_cause_infinite_scroll() {
    ...
    assert!(
        !tracker.detect_infinite_scroll(),
...
fn test_double_nested_ui_allocations() {
    ...
    println!(
        "Found {} allocate_ui_with_layout calls",
        allocate_ui_with_layout_count
    );
    println!(
        "Found {} allocate_ui_at_rect calls",
        allocate_ui_at_rect_count
    );

    // Check for nested allocation pattern (both in show and show_gantt_view)
    let show_method = content
        .split("fn show(")
        .nth(1)
        .unwrap_or("")
        .split("fn show_gantt_view")
        .nth(0)
        .unwrap_or("");
    let gantt_method = content
        .split("fn show_gantt_view")
        .nth(1)
        .unwrap_or("")
        .split("fn show_list_view")
        .nth(0)
        .unwrap_or("");
    ...
    if show_has_allocate && gantt_has_allocate {
        ...
        println!(
            "  - show_gantt_view() contains allocate_ui: {}",
            gantt_has_allocate
        );
    ...
    assert!(
        !(show_has_allocate && gantt_has_allocate),
...
fn test_all_allocations_are_bounded() {
        ...
        if line.contains("allocate_painter") {
            ...
            for ctx_line in context {
                ...
                if ctx_line.contains(".min(") {
                    ...
                    if ctx_line.contains("width")
                        || ctx_line.contains("height")
                        || ctx_line.contains("chart_width")
                        || ctx_line.contains("chart_height")
                    {
                        found_bounded = true;
                    }
                }
            ...
            if found_vec2 && !found_bounded {
                ...
                println!(
                    "Line {}: Potentially unbounded allocation: {}",
                    i + 1,
                    line.trim()
                );
            }
    ...
    assert!(
        unbounded_allocations.is_empty(),
        "Found {} potentially unbounded allocations that could cause infinite scroll:\n{:?}",
        unbounded_allocations.len(),
        unbounded_allocations
    );
}
...
fn test_no_recursive_rendering() {
    ...
    assert!(
        total_renders <= 5,
        "Too many render calls ({})! Possible recursive rendering.",
        total_renders
    );
    assert!(
        max_observed_depth <= 3,
        "Recursion depth too high ({})! Possible infinite recursion.",
        max_observed_depth
    );
}

fn simulate_render_frame(
