...
use plon::domain::task::{Priority, TaskStatus};
...
async fn test_max_occurrences_limit() {
    ...
    println!(
        "Final template occurrences: {}, active: {}, max: {:?}",
        final_template.recurrence_rule.occurrences_count,
        final_template.active,
        final_template.recurrence_rule.max_occurrences
    );
    assert_eq!(
        final_template.recurrence_rule.occurrences_count, 3,
        "Expected 3 occurrences"
    );
    assert!(
        !final_template.active,
        "Template should be inactive after reaching max occurrences"
    );
}
...
async fn test_template_with_metadata() {
    ...
    template
        .metadata
        .insert("project".to_string(), "backend".to_string());
    template
        .metadata
        .insert("sprint_length".to_string(), "2_weeks".to_string());
    ...
    assert_eq!(
        task.metadata.get("sprint_length"),
        Some(&"2_weeks".to_string())
    );
