use plon::domain::task::{Task, TaskStatus, SubTask};
use plon::domain::goal::Goal;
use plon::services::summarization::{SummarizationService, SummaryCache, SummarizationLevel};
...
async fn test_zoom_level_to_abstraction_mapping() {
    ...
    for (zoom, expected_level) in test_cases {
        ...
        assert_eq!(
            level, 
            expected_level,
            "Zoom {} should map to {:?}", zoom, expected_level
        );
    }
}
...
async fn test_dynamic_summarization_on_zoom() {
    ...
    // Create sample tasks
    let tasks = vec![
        create_task_with_details("Implement user authentication", "Security", vec![
            "Set up JWT tokens",
            "Create login endpoint",
            "Add password hashing",
            "Implement session management"
        ]),
        create_task_with_details("Design database schema", "Architecture", vec![
            "Define user table",
            "Create relations",
            "Add indexes",
            "Set up migrations"
        ]),
        create_task_with_details("Build UI components", "Frontend", vec![
    ...
    for zoom in zoom_levels {
        ...
        for task in &tasks {
            let summary = summarization_service.summarize_with_cache(
                &mut cache,
                task.id,
...
async fn test_summarization_caching() {
    ...
    let summary1 = service.summarize_with_cache(&mut cache, task_id, content, level).await;
    let first_duration = start.elapsed();
    
    // Second request - should hit cache
    let start = std::time::Instant::now();
    let summary2 = service.summarize_with_cache(&mut cache, task_id, content, level).await;
    let cached_duration = start.elapsed();
    
    // Cache should be faster
    assert!(cached_duration < first_duration / 10, "Cached request should be much faster");
...
async fn test_cluster_summarization() {
    ...
    assert!(cluster_summary.len() > 0, "Cluster summary should not be empty");
    assert!(cluster_summary.len() < 300, "High-level cluster summary should be reasonably concise");
...
async fn test_button_zoom_summarization() {
    ...
    for _ in 0..5 {
        ...
        let avg_length: usize = summaries.iter().map(|s| s.len()).sum::<usize>() / summaries.len().max(1);
...
async fn test_goal_summarization_at_different_levels() {
    ...
    let mut goal = Goal::new(
        ...
        "Launch new product features for Q1 including user dashboard, analytics, and reporting".to_string()
    );
    ...
    for level in levels {
        let summary = service.summarize_goal(&goal, &associated_tasks, level).await;
        
        match level {
            SummarizationLevel::HighLevel => {
                assert!(summary.len() < 200, "High-level goal summary should be concise");
            },
            SummarizationLevel::Detailed => {
                assert!(summary.len() > 50, "Detailed summary should be comprehensive");
            },
            _ => {}
        }
    }
...
async fn test_language_model_integration() {
    ...
    let high_summary = service.summarize(complex_description, SummarizationLevel::HighLevel).await;
    let mid_summary = service.summarize(complex_description, SummarizationLevel::MidLevel).await;
    let low_summary = service.summarize(complex_description, SummarizationLevel::LowLevel).await;
    
    // Verify summaries are progressively more detailed
    assert!(high_summary.len() < mid_summary.len(), "High-level should be shortest");
    assert!(mid_summary.len() < low_summary.len(), "Mid-level should be shorter than low-level");
    
    // Verify key concepts are preserved
    assert!(high_summary.to_lowercase().contains("auth"), "Should mention authentication");
}
...
async fn test_concurrent_summarization_requests() {
    ...
    let summarization_futures: Vec<_> = tasks.iter().map(|task| {
        let svc = service.clone();
        let desc = task.description.clone();
        async move {
            svc.summarize(&desc, SummarizationLevel::MidLevel).await
        }
    }).collect();
    ...
    assert!(results.is_ok(), "Concurrent requests should complete within timeout");
    
    let summaries = results.unwrap();
    assert_eq!(summaries.len(), tasks.len(), "Should get summary for each task");
}
...
fn create_task_with_details(title: &str, category: &str, subtasks: Vec<&str>) -> Task {
    ...
    task.metadata.insert("category".to_string(), category.to_string());
...
fn create_sample_tasks(count: usize) -> Vec<Task> {
    (0..count)
        .map(|i| create_task(&format!("Task {}", i), &format!("Description for task {}", i)))
        .collect()
}
...
fn verify_summary_matches_level(summary: &str, level: SummarizationLevel) {
    match level {
        SummarizationLevel::HighLevel => {
            assert!(summary.len() < 60, "High-level summary should be under 60 chars");
        },
        SummarizationLevel::MidLevel => {
            assert!(summary.len() < 150, "Mid-level summary should be under 150 chars");
        },
        SummarizationLevel::LowLevel => {
            assert!(summary.len() < 300, "Low-level summary should be under 300 chars");
        },
        SummarizationLevel::Detailed => {
