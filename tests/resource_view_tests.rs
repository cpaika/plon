...
mod resource_view_tests {
    ...
    fn test_edit_resource_fields() {
        ...
        if let Some(resource) = resources.iter_mut().find(|r| r.id == view.selected_resource.unwrap()) {
        ...
        assert_eq!(resources[0].email, Some("alice.smith@example.com".to_string()));
...
mod resource_view_integration_tests {
    ...
    fn test_complete_edit_workflow() {
        let mut view = ResourceView::new();
        let mut resources = vec![
            create_test_resource("John Doe", "Junior Developer", 40.0),
        ];
