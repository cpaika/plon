...
fn test_timeline_infinite_scroll_runtime_detection() {
    ...
    println!("Average pixel change: {:.2}%", analysis.average_pixel_change * 100.0);
    println!("Maximum pixel change: {:.2}%", analysis.max_pixel_change * 100.0);
    println!("Consistent movement detected: {}", analysis.consistent_movement);
    ...
    assert!(!analysis.infinite_scroll_detected, 
...
fn analyze_for_infinite_scroll(screenshots: &[String]) -> ScrollAnalysis {
    ...
    for i in 1..screenshots.len() {
        if let Ok((change_ratio, h_move, v_move)) = compare_frames(&screenshots[i-1], &screenshots[i]) {
            pixel_changes.push(change_ratio);
            horizontal_movements.push(h_move);
            vertical_movements.push(v_move);
            
            println!("Frame {}->{}: {:.2}% change, H:{}, V:{}", 
                     i-1, i, change_ratio * 100.0, h_move, v_move);
        }
    }
...
fn find_pixel_shift(img1: &DynamicImage, img2: &DynamicImage, x: u32, y: u32, radius: u32) 
    -> Option<(i32, i32)> {
    let pixel1 = img1.get_pixel(x, y);
    let (width, height) = img1.dimensions();
