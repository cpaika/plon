...
fn test_map_view_has_panning_methods() {
    ...
    assert!(map_view.get_zoom_level() > 0.0, "Zoom level should be positive");
}
...
fn test_map_view_zoom_limits() {
    ...
    assert!(map_view.get_zoom_level() >= 0.1, "Zoom should be clamped to minimum");
    
    map_view.set_zoom_level(100.0); // Very large
    assert!(map_view.get_zoom_level() <= 5.0, "Zoom should be clamped to maximum");
    
    // Test normal zoom value
    map_view.set_zoom_level(2.0);
    assert_eq!(map_view.get_zoom_level(), 2.0, "Normal zoom should be set correctly");
}