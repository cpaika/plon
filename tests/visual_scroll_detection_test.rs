...
fn capture_app_window(output_path: &str) {
    ...
    if !result.status.success() {
        println!("Screenshot failed: {:?}", String::from_utf8_lossy(&result.stderr));
    }
}
...
fn analyze_screenshot_sequence(screenshots: &[String]) -> ScrollAnalysisResult {
    ...
    for i in 1..screenshots.len() {
        if let Ok(motion) = detect_motion_between_frames(&screenshots[i-1], &screenshots[i]) {
            ...
            println!("Frame {}->{}: {:.2}% pixels changed, v_shift={}, h_shift={}", 
                     i-1, i, motion.pixel_change_ratio * 100.0,
                     motion.vertical_shift, motion.horizontal_shift);
        }
    }
...
fn detect_motion_between_frames(path1: &str, path2: &str) -> Result<MotionData, Box<dyn std::error::Error>> {
    // Load images
    ...
    let (avg_h_shift, avg_v_shift) = if !shift_samples.is_empty() {
        ...
        (sum_h / shift_samples.len() as i32, sum_v / shift_samples.len() as i32)
    } else {
        (0, 0)
    };
...
fn find_pixel_shift(img1: &image::RgbaImage, img2: &image::RgbaImage, 
                    x: u32, y: u32, search_radius: u32) -> Option<(i32, i32)> {
